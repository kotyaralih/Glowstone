...
namespace pocketmine {
    ...
    if (version_compare("7.0", PHP_VERSION) > 0) {
        echo "[Glowstone] You must use PHP >= 7.0" . PHP_EOL;
        echo "[Glowstone] Please use the installer provided on the homepage." . PHP_EOL;
        exit(1);
    }

    if (!extension_loaded("pthreads")) {
        echo "[Glowstone] Unable to find the pthreads extension." . PHP_EOL;
        echo "[Glowstone] Please use the installer provided on the homepage." . PHP_EOL;
        exit(1);
    }
    ...
    if (GlowstoneInfo::NAME !== "Glowstone") {
        echo "[Glowstone] Unable to start. Server name doesn't match original." . PHP_EOL;
        exit(1);
    }

    const SERVER_NAME = GlowstoneInfo::NAME;
    const VERSION = GlowstoneInfo::VERSION;
    const API_VERSION = GlowstoneInfo::API;
    const CODENAME = GlowstoneInfo::CODENAME;
    const MINECRAFT_VERSION = GlowstoneInfo::MINECRAFT_VERSION;
    const MINECRAFT_VERSION_NETWORK = GlowstoneInfo::MINECRAFT_VERSION_NETWORK;
    const GLOWSTONE_API_VERSION = GlowstoneInfo::GLOWSTONE_API;
    ...
    $logger->info("Stopping Glowstone");

    foreach (ThreadManager::getInstance()->getAll() as $id => $thread) {
        $logger->debug("Stopping " . (new \ReflectionClass($thread))->getShortName() . " потока");
        $thread->quit();
    }
    ...
    echo "Server stopped." . Terminal::$FORMAT_RESET . "\n";
