...
class StatusCommand extends VanillaCommand
{
    ...
    public function execute(CommandSender $sender, $commandLabel, array $args)
    {
        ...
        $sender->sendMessage(TextFormat::GREEN . "---- " . TextFormat::WHITE . "Glowstone server status" . TextFormat::GREEN . " ----");
        ...
        $tpsColorAverage = TextFormat::GREEN;
        if ($server->getTicksPerSecondAverage() < 17) {
            $tpsColorAverage = TextFormat::GOLD;
        } elseif ($server->getTicksPerSecondAverage() < 12) {
            $tpsColorAverage = TextFormat::RED;
        }

        $sender->sendMessage(TextFormat::GOLD . "Current TPS: " . $tpsColor . $server->getTicksPerSecond() . " (" . $server->getTickUsage() . "%)");
        $sender->sendMessage(TextFormat::GOLD . "Average TPS: " . $tpsColorAverage . $server->getTicksPerSecondAverage() . " (" . $server->getTickUsageAverage() . "%)");
