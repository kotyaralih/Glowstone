...
class MemoryManager
{
    ...
    {
        $this->memoryLimit = ((int)$this->server->getProperty("memory.main-limit", 0)) * 4096 * 4096;
        ...
        $this->globalMemoryLimit = ((int)$this->server->getProperty("memory.global-limit", 0)) * 4096 * 4096;
    ...
    {
        $this->server->getLogger()->debug("[Memory Manager] " . ($global ? "Global " : "") . "Low memory triggered, limit " . round(($limit / 4096) / 4096, 2) . "MB, using " . round(($memory / 4096) / 4096, 2) . "MB");
        ...
        $this->server->getLogger()->debug("[Memory Manager] Freed " . round(($ev->getMemoryFreed() / 4096) / 4096, 2) . "MB, $cycles cycles");
    }
