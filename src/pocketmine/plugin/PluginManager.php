                ...
                foreach (new \RegexIterator(new \DirectoryIterator($directory), $loader->getPluginFilters()) as $file) {
                    ...
                    try {
                        ...
                        if ($description instanceof PluginDescription) {
                            ...
                            $compatibleglowapi = false;
                            foreach ($description->getCompatibleGlowApis() as $version) {
                                //Format: majorVersion.minorVersion.patch
                                $version = array_map("intval", explode(".", $version));
                                $apiVersion = array_map("intval", explode(".", $this->server->getGlowApiVersion()));
                                //Completely different API version
                                if ($version[0] > $apiVersion[0]) {
                                    continue;
                                }
                            ...
                            if ($compatibleglowapi === false) {
                                $this->server->getLogger()->error("Could not load plugin '{$description->getName()}': Incompatible GlowAPI version");
                                continue;
                            }
