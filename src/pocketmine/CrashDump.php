...
class CrashDump
{
        ...
        if (strpos($error["file"], "src/pocketmine/") === false and strpos($error["file"], "src/raklib/") === false and file_exists($error["fullFile"])) {
            ...
            foreach ($this->server->getPluginManager()->getPlugins() as $plugin) {
                ...
                if (strpos($error["file"], $filePath) === 0) {
                    ...
                    $this->addLine("Plugin: " . $plugin->getDescription()->getFullName());
                    break;
                }
            }
        } else {
    ...
    private function generalData()
    {
        ...
        $this->addLine("Glowstone version: " . $version->get(false) . " #" . $version->getBuild() . " [Protocol " . Info::CURRENT_PROTOCOL . "; API " . API_VERSION . "]");
